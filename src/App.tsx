  ...
  return (
    <Router>
      <div className="min-h-screen bg-pink-50 text-gray-800 font-sans">
        {/* Header */}
        <header className="bg-white border-b border-pink-100 p-6 sticky top-0 z-10 shadow-sm">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-pink-500 tracking-tight flex items-center">
                <School className="mr-2 w-8 h-8" /> ระบบเช็คชื่อนักเรียนออนไลน์
              </h1>
              <div className="flex gap-4 mt-2">
                <input 
                  ...
                  className="bg-transparent border-none text-sm text-gray-500 focus:ring-0 p-0 hover:text-pink-400 transition-colors"
                  placeholder="โรงเรียนบ้านคลองสารเพชร"
                />
                <input 
                  type="text" 
                  value={teacherName} 
            ...
            <nav className="flex overflow-x-auto gap-2 p-1 bg-pink-50 rounded-2xl">
              <Link to="/" className="flex items-center px-4 py-2 rounded-xl text-sm font-bold hover:bg-white hover:text-pink-500 transition-all">
                <ClipboardList className="w-4 h-4 mr-2" /> เช็คชื่อ
              </Link>
              <Link to="/report" className="flex items-center px-4 py-2 rounded-xl text-sm font-bold hover:bg-white hover:text-pink-500 transition-all">
                <FileText className="w-4 h-4 mr-2" /> รายงาน
              </Link>
              <Link to="/status" className="flex items-center px-4 py-2 rounded-xl text-sm font-bold hover:bg-white hover:text-pink-500 transition-all">
                <CheckCircle2 className="w-4 h-4 mr-2" /> สถานะ
              </Link>
              <Link to="/stats" className="flex items-center px-4 py-2 rounded-xl text-sm font-bold hover:bg-white hover:text-pink-500 transition-all">
                <BarChart3 className="w-4 h-4 mr-2" /> สถิติ
              </Link>
              <Link to="/manage" className="flex items-center px-4 py-2 rounded-xl text-sm font-bold hover:bg-white hover:text-pink-500 transition-all">
                <Users className="w-4 h-4 mr-2" /> จัดการนักเรียน
              </Link>
            </nav>
          </div>
        </header>
